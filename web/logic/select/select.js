    ...
    $.fn.mySelect = function (options) {
       ...
       //生成option-item并追加展示
        var html = '';
 		...
 		html += '<div class="select-picker-search-checked">Choose</div>';
 		html += '</div>';
 		...
 		$this.find("option").each(function(){ 
 			...
 			html += '<span data-val='+_this.val()+'>'+_this.text()+'</span>';
 			html += '</div>';
 		})
 		html += '</div>';
 		html += '</div>'; 
 		...
 		$this.on('click',".select-picker-options-list-item",function(){ 
			...
			$this.find(".select-picker-options-list-item").each(function(){ 
				...
				if(_this.find('.duihao-checked').length > 0){ 
					...
					checkedArrVal.push($.trim(_this.find("span").data("val")))
			...
			if (checkedArr.length > 0) {
			    ...
			    $this.find('.select-picker-search-checked').text('Choose').css('color', '#757575');
			}
 		})
